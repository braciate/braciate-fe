...
const TeamMemberCard: FC<TeamMember> = ({ name, linkedin, instagram }) => (
  <div className="flex gap-2 sm:gap-6 my-4">
    <div className="border-2 p-0.5 sm:p-4 rounded-full bg-white text-center w-full flex gap-2 justify-center text-black text-lg sm:text-xl md:text-2xl items-center">
      <h2>{name}</h2>
    </div>
    <div className="flex items-center gap-2 sm:gap-4">
      <Link
        href={instagram}
        target="_blank"
        className={`flex items-center justify-center rounded-full bg-white p-5 text-black w-12 h-12 sm:w-16 sm:h-16 ${styles.footer} fill-black hover:fill-white`}
      >
...
const TeamSection: FC<{ title: string; role: string }> = ({ title, role }) => (
  <section className="mt-10">
    <h2
      className={`text-3xl sm:text-4xl md:text-5xl font-bold mb-4 p-2 sm:p-4 text-white rounded-full font-jaoren w-5/12 ${text.subtitle}`}
    >
      {title}
    </h2>
    {teamMembers
